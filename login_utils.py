...
def check_password(email, password):
    ...
    if not fetched_login:
        ...
        update_failed_login_attempt = text(
            "UPDATE logins SET attempt_count=attempt_count+1 WHERE user_id=:user_id")
        g.db.execute(update_failed_login_attempt,
                     {"user_id": fetched_login[0]['user_id']})
        if fetched_login[0]['attempt_count'] >= 3:
            disable_account = text(
                "UPDATE logins SET account_disabled=True WHERE user_id=:user_id")
            g.db.execute(disable_account,
                         {"user_id": fetched_login[0]['user_id']})
        return "Wrong Username or Password"
    elif email == fetched_login[0]['email'] and check_password_hash(fetched_login[0]['password'], password):
        reset_attempts_upon_success = text(
            "UPDATE logins SET account_disabled=False, attempt_count=0 WHERE user_id=:user_id")
